...
import type { Contexto, ContextoVisualizacao, EmprestimoParcela, EmprestimoResumo } from "../types";
...
export function EmprestimosPage({
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      setItems(await listEmprestimos(contexto));
      onChanged?.();
    } catch (err) {
  ...
  async function handleDelete(id: number) {
    if (!(await confirm("Excluir este empréstimo e todas as parcelas?"))) return;
    try {
  ...
  return (
    <div>
      {!embedded && (
        <PageHeader
          title="Empréstimos"
          subtitle="Empréstimos bancários e pessoais — parcelas e saldo devedor"
          action={
            <Button
              onClick={() => {
                setEditing(null);
      ...
      {loading || ctxLoading ? (
        ...
        <EmptyState message="Nenhum empréstimo cadastrado." />
      ) : (
